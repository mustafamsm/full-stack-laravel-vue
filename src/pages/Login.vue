<template>
    <GuestLayout>
        ...
        <div v-if="errorMessage" class="mt-4 text-center py-2 px-3 rounded text-white bg-red-400 ">
            {{ errorMessage }}
        </div>
        <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
            ...
            <p class="mt-10 text-center text-sm/6 text-gray-500">
                Not a member?
                {{ ' ' }}
                <RouterLink :to="{ name: 'Signup' }" class="font-semibold text-indigo-600 hover:text-indigo-500">Create
                    an
                    account</RouterLink>
            </p>
        </div>
...
const submit = async () => {
    ...
    await axiosClient.post('/login', data.value).then(() => {
